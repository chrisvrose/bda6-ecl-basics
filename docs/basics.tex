...
Result 2 and 'projections' will look similar, and that countofYoung is zero. So that's interesting. Here's the rest of the outputs:

\begin{figure}[h]
    \centering
    \includegraphics[width=.7\linewidth]{../output/26/1.png}
    \caption{Result 1 - Output of a segment}
\end{figure}

\begin{figure}[h]
    \centering
    \includegraphics[width=.8\linewidth]{../output/26/2.png}
    \caption{Result 2 - Output, filtering columns}
\end{figure}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=.8\linewidth]{../output/26/6withid.png}
...
Filtering by the nth record is not as unusual as it might look. In ECL, datasets, statements may not may not be order-sensitive, and can be configured as such. This means that the order of the dataset, cannot be ignored, and can be used.
...
Functions, unfortunately, may not be called from itself in ECL. This puts out the idea of recursion, in ECL.
...
As there are lots of output, the output is shown in xml format.

\lstinputlisting[caption={(P8) Outputs in XML}]{../output/62/outputs.xml}
...
\begin{figure}[h]
    ...
    \caption{(P7) Output 1 - Information about the youngest few people in this dataset}
\end{figure}

\begin{figure}[h]
    \centering
    \includegraphics[width=.5\linewidth]{../output/63/2}
    \caption{(P7) Output 2 - A grouped report of age-wise counts of heart disease}
\end{figure}
\begin{figure}[h]
    \centering
    \includegraphics[width=.75\linewidth]{../output/63/3}
    \caption{(P7) Output 3 - The age and the resting heart rate, and whether they had a heart disease, ranked by heart rate}
\end{figure}
...
% 1
The \lstinline!Visualizer! is a module that is used for Visualizations, and 
